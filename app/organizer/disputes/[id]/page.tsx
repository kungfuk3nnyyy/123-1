...
interface DisputeDetails {
  ...
  User: { name: string | null }; // User who raised it
  Booking: {
    id: string;
    amount: number;
    Event: {
...
async function getDisputeDetails(disputeId: string): Promise<DisputeDetails | null> {
      ...
      include: {
        ...
        Booking: {
          include: {
            ...
            User_Booking_talentIdToUser: { select: { id: true, name: true, email: true, image: true } },
            User_Booking_organizerIdToUser: { select: { id: true } },
          },
        },
      },
    });
    ...
    // Security check: Ensure the logged-in user is the organizer of this booking
    if (dispute.Booking.User_Booking_organizerIdToUser.id !== session.user.id) {
...
export default async function DisputeDetailPage({ params }: { params: { id: string } }) {
  ...
  const talent = dispute.Booking.User_Booking_talentIdToUser;

  return (
    <div className="space-y-6">
      <Button variant="ghost" asChild>
        <Link href="/organizer/disputes">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
             <Card>
                <CardHeader>
                     <CardTitle className="text-base flex items-center"><User className="h-5 w-5 mr-2" /> Talent</CardTitle>
                </CardHeader>
